...
- **골드 스탬프 북**: 지도상 명소를 터치 및 스핀(Spin)하면 경험치(+50 XP)를 획득하고, 획득한 스탬프를 전용 화면에서 확인할 수 있습니다. 앱에 등록된 명소 전체를 대상으로 하며(과거 6개 랜드마크로 고정됐던 것에서 확장), 각 스탬프는 실제 랜드마크 사진으로 표시되고 미획득 스탬프는 흑백 처리 + 자물쇠 오버레이로 구분됩니다. 스탬프를 탭하면 역사적 사실·관람 팁과 함께 여행 보고서 데이터를 이용한 최초 방문 시각을 볼 수 있고, 우측 상단 공유 버튼으로 스탬프북 전체를 이미지로 캡처해 공유할 수 있습니다.
...
- **재방문 사용자 온보딩 스킵**: 언어 선택과 캐릭터 선택을 한 번 마치면 Firestore에 기록되어, 다음 실행부터는 랜딩 화면 이후 바로 홈 화면으로 진입합니다 (언어/캐릭터/닉네임도 저장된 값으로 자동 복원). 처음 쓰는 사용자이거나 Firebase 미설정 상태면 기존처럼 언어 선택부터 정상적으로 진행됩니다.
...
- **소셜(친구/파티) 탭 다국어 완성**: 한국어/영어에만 있던 친구·파티 관련 문구 약 50개를 일본어·중국어(간체)·베트남어·태국어까지 전부 채워 넣어, 해당 언어 사용 시 소셜 탭 전체가 영어로만 보이던 문제를 해결했습니다.
- **명소 정보 베트남어/태국어 완성**: 전체 14개 명소의 역사적 사실·관람 팁에 베트남어·태국어가 아예 없어 한국어로 표시되던 문제를 고쳐, 이름과 함께 완전히 현지어로 표시됩니다.
- **AI 비서 버튼 폭 제한**: 일본어 등 라벨이 긴 언어에서 AI 비서 버튼이 가운데 지도 버튼을 침범하던 문제를, 폭을 제한하고 필요 시 폰트를 자동으로 줄여 해결했습니다.
- **온보딩 뒤로가기 수정**: 언어 선택 후 캐릭터 선택 화면에서 뒤로가기를 누르면 앱이 그냥 종료되던 문제를 고쳐, 언어를 잘못 골랐을 때 언어 선택 화면으로 정상적으로 돌아갈 수 있습니다.
- **사용법 튜토리얼**: 캐릭터 선택 이후 지도 탐험·퀘스트/스탬프북·소셜/위치공유·안전정보를 5장의 슬라이드로 간단히 안내합니다. 설정 화면에서 "튜토리얼 다시보기"로 언제든 재확인할 수 있습니다.
- **Mapbox 로고 위치 조정**: 좌하단 AI 비서 버튼에 Mapbox 로고/저작권 표시가 가려지던 문제를, 우하단으로 옮겨 항상 온전히 보이도록 수정했습니다 (Mapbox 약관상 로고 자체를 숨기는 것은 별도 협의 없이는 불가능합니다).
- **게임모드 상태바 가림 수정**: Mapbox 기본 나침반/축척 표시가 화면 상단(0,0) 기준으로 그려져 상태바(시간·와이파이·배터리)와 겹치던 문제를 상태바 높이만큼 여백을 줘서 해결했습니다.
- **랜딩 화면 최소 노출 시간 보장**: 데이터 로딩이 너무 빨리 끝나면 랜딩 화면이 순간적으로 지나가버리던 문제를 진행바 애니메이션 시간을 2초로 늘려 해결했습니다.
...
- **Sharing**: share_plus (스탬프북 이미지 공유), path_provider (임시 파일 경로)
...
│   ├── tutorial_screen.dart  # 사용법 튜토리얼 (온보딩 마지막 단계 + 설정에서 다시보기)
...
│   ├── stamp_book_screen.dart # 스탬프 북 - 전체 명소 수집 현황, 방문 상세, 이미지 공유
...
> `flutterfire configure`를 아직 실행하지 않았다면 친구 목록은 "친구 없음" 상태로, 내 QR 코드 보기는 안내 스낵바로 표시됩니다(예전에는 무한 로딩/무반응이었으나 수정됨). 그 외 지도·퀘스트·도슨트 등 나머지 기능은 정상 동작합니다.
